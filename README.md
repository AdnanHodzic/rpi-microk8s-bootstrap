...
For full list of changes that will be made, please refer to ["microk8s-bootstrap" Terraform module README](./modules/microk8s-bootstrap/README.md).
...
Without this project, upgrading to a new Kubernetes (MicroK8s) version is a daunting (manual) task which involve:

* Get a list of nodes of your K8s cluster
* Drain worker nodes
* Verify no workloads are running on target node
* Upgrade Kubernetes (MicroK8s) on target node
* Verify upgrade went well and then resume pod scheduling on the upgraded node
...
As part of ["microk8s-bootstrap" module](./modules/microk8s-bootstrap/README.md) execution, [microk8s-bootstrap.tpl](./modules/microk8s-bootstrap/microk8s_bootstrap.tpl) file will generate "rpi-microk8s-boostrap.py" file during Terraform code execution that has all necessary changes, which is designed to be run numerous times without overwriting existing configurations. Meaning, this code will run *every time* and as such won't need to rely on functionality of Terraform state file. Hence, undocumented terraform ["inmem" backend](./main.tf) is used, which will execute terraform code without generating terraform.tfstate file. To override this behavior, comment its code block.
...
For more information, please refer to [rpi-microk8s-bootstrap/main.tf](./main.tf) or ["microk8s-bootstrap" module README](./modules/microk8s-bootstrap/README.md) file. After successful Terraform code execution, please refer to:
...
You want to identify what the cluster leader (master) node, by SSH-ing to one of the nodes and running following command, i.e:
``` 
...
1. ["microk8s-bootstrap" module](./modules/microk8s-bootstrap/README.md) will setup SSH key based authentication if SSH key was found on local system, hence if this was done value provided in `password` variable won't have any effect since authentication will be based on SSH key. 
